function getRaces( pg ) {
	$.ajax({
		type : "GET",
		url : apiUrl + "/pastraces?page=" + pg + "&pageSize=20",
		processData : true,
		data : {},
		dataType : "jsonp",
...
function renderMonthlies ( data ) {
	...
	for ( x in data[ 'monthlyStats' ] ) {
		...
		var percentageFilled = (stat.totalRaces / biggestmonth) * 100;
...
function hashChange () { //detect hash change, and update appropriately.
...
$( document ).ready( function(){
	...
	if (!window.location.hash) {
		...
		hashChange(); // first time the page loads
	}
